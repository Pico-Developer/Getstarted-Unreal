...
void FPICOXRInput::ProcessButtonEvent()
{
	...
	if (bHandTrackingAvailable)
	{
		...
		for (int Hand =0;Hand<2;Hand++)
		{
			...
			{
				MessageHandler->OnControllerButtonPressed(HandButtons[Hand][PinchIndex], 0, false);
			}
			else if (HandButtonState[Hand][PinchIndex]==0&&HandButtonState[Hand][PinchIndex]!=LastHandButtonState[Hand][PinchIndex])
			{
				MessageHandler->OnControllerButtonReleased(HandButtons[Hand][PinchIndex], 0, false);
			}
...
void FPICOXRInput::ProcessButtonAxis()
{
	...
	if (bHandTrackingAvailable)
	{
		MessageHandler->OnControllerAnalog(FPICOKeyNames::PICOHand_Left_PinchStrength, 0, GetPinchStrength(EPICOXRHandType::HandLeft));
		MessageHandler->OnControllerAnalog(FPICOKeyNames::PICOHand_Right_PinchStrength, 0, GetPinchStrength(EPICOXRHandType::HandRight));
	}
}
