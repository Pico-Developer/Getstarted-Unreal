...
FPICOXRStereoLayer::FPICOXRStereoLayer(FPICOXRHMD* InHMDDevice, uint32 InPXRLayerId, const IStereoLayers::FLayerDesc& InDesc)
	...
	, bExternalSurface(false)
	, bIsDRM(false)
#ifdef PICO_CUSTOM_ENGINE
	, bEnableEyeTrackingFoveationRendering(false)
#endif
	, SurfaceType(EPICOXRAndroidSurface3DType::LeftRight)
...
FPICOXRStereoLayer::FPICOXRStereoLayer(const FPICOXRStereoLayer& InPXRLayer)
	...
	, bExternalSurface(InPXRLayer.bExternalSurface)
	, bIsDRM(InPXRLayer.bIsDRM)
#ifdef PICO_CUSTOM_ENGINE
	, bEnableEyeTrackingFoveationRendering(InPXRLayer.bEnableEyeTrackingFoveationRendering)
#endif
	, SurfaceType(InPXRLayer.SurfaceType)
...
#endif
	, ExternalSurface_RenderThreadValid(InPXRLayer.ExternalSurface_RenderThreadValid)
...
bool FPICOXRStereoLayer::InitPXRLayer_RenderThread(const FGameSettings* Settings, FPICOXRRenderBridge* CustomPresent, FDelayDeleteLayerManager* DelayDeletion, FRHICommandListImmediate& RHICmdList, const FPICOXRStereoLayer* InLayer)
{
	...
	else
	{
		...
		if (bExternalSurface)
		{
			...
			PxrLayerCreateParam.sampleCount = 1;
			PxrLayerCreateParam.mipmapCount = 1;
			PxrLayerCreateParam.layerFlags = PXR_LAYER_FLAG_ANDROID_SURFACE;

			if (bIsDRM)
			{
				PxrLayerCreateParam.layerFlags |= PXR_LAYER_FLAG_PROTECTED_CONTENT;
			}

			switch (SurfaceType)
			{
			case EPICOXRAndroidSurface3DType::LeftRight:
				PxrLayerCreateParam.layerFlags |= PXR_LAYER_FLAG_3D_LEFT_RIGHT_SURFACE;
				break;
			case EPICOXRAndroidSurface3DType::TopBottom:
				PxrLayerCreateParam.layerFlags |= PXR_LAYER_FLAG_3D_TOP_BOTTOM_SURFACE;
				break;
			default:
				break;
			}

			PxrLayerCreateParam.layerLayout = PXR_LAYER_LAYOUT_MONO;
		}
	}
	...
	if (IfCanReuseLayers(InLayer))
	{
		...
		ExternalSurface_RenderThreadValid = InLayer->ExternalSurface_RenderThreadValid;
	}
    else
	{
		...
		bool bExternalSurfaceCreated = false;
#if PLATFORM_ANDROID
		jobject* Surface = nullptr;
		if (bExternalSurface && !ExternalSurface_RenderThreadValid.IsValid())
		{
			ExternalSurface_RenderThreadValid = MakeShareable<FPxrExternalSurface>(new FPxrExternalSurface());
			Surface = ExternalSurface_RenderThreadValid->GetSurface();
		}
#endif
#ifdef PICO_CUSTOM_ENGINE
		...
		ExecuteOnRHIThread([&]()
			{
				...
				if (FPICOXRHMDModule::GetPluginWrapper().bIsSessionInitialized && (FPICOXRHMDModule::GetPluginWrapper().CreateLayer(&PxrLayerCreateParam) == 0))
				{
					...
					{
#if PLATFORM_ANDROID
						FPICOXRHMDModule::GetPluginWrapper().GetLayerAndroidSurface(PxrLayerID, PXR_EYE_RIGHT, Surface);
#endif
						bExternalSurfaceCreated = true;
					}
				}
				else
				{
		...
		else if (bExternalSurfaceCreated)
		{
			PxrLayer = MakeShareable<FPxrLayer>(new FPxrLayer(ID, PxrLayerID, DelayDeletion));
			PXR_LOGI(PxrUnreal, "External Surface Created:%u", PxrLayerID);
		}
		else
		{
...
void FPICOXRStereoLayer::ReleaseResources_RHIThread()
{
	...
	LeftSwapChain.Reset();
	bTextureNeedUpdate = false;
	ExternalSurface_RenderThreadValid.Reset();
}
...
#if PLATFORM_ANDROID
jobject* FPICOXRStereoLayer::GetAndroidExternalSurface()
{
	check(InRenderThread() || InRHIThread());
	jobject* surface = nullptr;
	if (ExternalSurface_RenderThreadValid.IsValid())
	{
		surface = ExternalSurface_RenderThreadValid->GetSurface();
	}
	return surface;
...
const void FPICOXRStereoLayer::SubmitLayer_RHIThread(const FGameSettings* Settings, const FPXRGameFrame* Frame)
{
	...
	// If the layer being submitted is the Eye Layer
	if (ID == 0)
	{
		...
		else if (ShapeType == (int32)PxrLayerShape::PXR_LAYER_EAC)
		{
			...
			for (int32 EyeIndex = 0; EyeIndex < 2; EyeIndex++)
			{
				layerSubmit.header.imageRect[EyeIndex] = {
					FGenericPlatformMath::RoundToInt(SizeX * EACProps.UVRect[EyeIndex].Min.X),
					FGenericPlatformMath::RoundToInt(SizeY * EACProps.UVRect[EyeIndex].Min.Y),
					FGenericPlatformMath::RoundToInt(SizeX * (EACProps.UVRect[EyeIndex].GetSize().X)),
					FGenericPlatformMath::RoundToInt(SizeY * (EACProps.UVRect[EyeIndex].GetSize().Y))
				};
				...
				layerSubmit.offset[EyeIndex] = { Offset.X, Offset.Y, Offset.Z };
				layerSubmit.offsetRot[EyeIndex] = { OffsetRot.X, OffsetRot.Y, OffsetRot.Z, OffsetRot.W };
			}
