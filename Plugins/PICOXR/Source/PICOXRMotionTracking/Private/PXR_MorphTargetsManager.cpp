...
void FPXRMorphTargetsManager::UpdateMeshMorphTargets(USkinnedMeshComponent* TargetMeshComponent)
{
	if (TargetMeshComponent && TargetMeshComponent->SkeletalMesh)
	{
		...
		{
			FAnimationRuntime::AppendActiveMorphTargets(TargetMeshComponent->SkeletalMesh, MeshMorphTargetCurves, TargetMeshComponent->ActiveMorphTargets, TargetMeshComponent->MorphTargetWeights);
		}
	}
}
