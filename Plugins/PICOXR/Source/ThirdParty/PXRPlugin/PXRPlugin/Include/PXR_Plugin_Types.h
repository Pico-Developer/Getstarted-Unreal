...
typedef struct PxrEyePupilInfo {
	float leftEyePupilDiameter; //返回瞳孔大小， 单位像素
	float rightEyePupilDiameter;
	float leftEyePupilPosition[2]; // 返回瞳孔位置，单位像素
...
typedef enum {
	PXR_BTM_NONE                =-1,
	PXR_BTM_WITH_SWIFT          = 0,
	PXR_BTM_WITHOUT_SWIFT       = 1,
	PXR_BTM_COUNT               = 2
}PxrBodyTrackingMode;

typedef int64_t PxrBodyTrackingGetDataFlags;
static const PxrBodyTrackingGetDataFlags PXR_BODY_POSE     = 1 << 0;
static const PxrBodyTrackingGetDataFlags PXR_BODY_ACTION   = 1 << 1;
static const PxrBodyTrackingGetDataFlags PXR_BODY_VELO_ACC = 1 << 2;
...
typedef struct PxrBodyTrackingState_ {
	...
	uint8_t                     connectedBandCount;
	uint8_t                     fitnessBand[12];
} PxrBodyTrackingState;

typedef struct PxrBodyTrackingGetDataInfo_ {
	int                         apiVersion;
	int64_t                     displayTime;
	PxrBodyTrackingGetDataFlags flags;
